			...
			callback: function (r) {
				...
				if (r && r.message) {
					...
					if (is_tracking == true) {
						...
						if (!map) {
							var map = new window.L.map('sales-map', mapOptions);
						}						
						...
						tracked_locations.forEach((m, index) => {
							...
							var routepoint = new window.L.Marker([m.lat, m.lng], routeOptions);
					...
					if (is_meeting == true) {
						...
						meeting_data.forEach((m, index) => {
							...
							var marker = new window.L.Marker(meeting_latlong, markerOptions);
