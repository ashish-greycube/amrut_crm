			...
			callback: function (r) {
				...
				if (r && r.message) {
					...
					if (is_tracking == false && is_meeting == false) {
						frm.get_field('visit_map_html').$wrapper.html(`<div id = "sales-map" style = "width: 1560px; height: 580px"></div>`)
					...
					if (is_tracking == true) {
						...
						tracked_locations.forEach((m, index) => {
							console.log(m.lat, m.lng, m.time)
					...
					if (is_tracking == false && is_meeting == true) {
						let latlong = JSON.parse(String(r.message[1][0]))
